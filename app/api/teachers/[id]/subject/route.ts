...
export async function PUT(req: NextRequest, { params }: { params: { id: string } }) {
  ...
  try {
    ...
    const td = await prisma.teacherDetail.findFirst({ where: { userId: teacherId, schoolId: ctx.schoolId } });
    if (!td) return NextResponse.json({ message: "Not found" }, { status: 404 });
    if (subjectId) {
      const subj = await prisma.subject.findUnique({ where: { id: subjectId } });
      if (!subj || subj.schoolId !== ctx.schoolId) return NextResponse.json({ message: "Invalid subject" }, { status: 400 });
    }
    await prisma.teacherDetail.updateMany({ where: { userId: teacherId, schoolId: ctx.schoolId }, data: { subjectId: subjectId ?? null } });
