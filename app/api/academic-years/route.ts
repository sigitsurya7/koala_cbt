...
export async function POST(req: NextRequest) {
  ...
  try {
    ...
    const { schoolId, label, startDate, endDate, isActive = false } = body ?? {};
    if (!schoolId || !label || !startDate || !endDate)
      return NextResponse.json({ message: "schoolId, label, startDate, endDate wajib" }, { status: 400 });
    const created = await prisma.academicYear.create({ data: { schoolId, label, startDate: new Date(startDate), endDate: new Date(endDate), isActive } });
    return NextResponse.json({ id: created.id });
  } catch (e: any) {
    return NextResponse.json({ message: e?.message ?? "Gagal membuat tahun ajaran" }, { status: 500 });
  }
}
