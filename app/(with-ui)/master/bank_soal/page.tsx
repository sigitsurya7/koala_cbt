...
export default function BankSoalPage() {
  ...
  const openCreateSubject = () => { setSubjectForm({ id: "", schoolId, departmentId: null, departmentName: null, name: "", grade: null } as any); setEditing(null); setSubjectOpen(true); };
  ...
  const saveSubject = async () => {
    ...
    try {
      if (editing) {
        await axios.put(`/api/subjects/${editing.id}`, { name: subjectForm.name, departmentId: subjectForm.departmentId || null, grade: subjectForm.grade ?? null });
        toast.success("Mapel diperbarui");
      } else {
        await axios.post(`/api/subjects`, { name: subjectForm.name, departmentId: subjectForm.departmentId || null, grade: subjectForm.grade ?? null });
    ...
    <div className="space-y-6">
      ...
      {schoolId && (
        <DataTable<SubjectRow>
            ...
            { key: "actions", header: "Aksi", render: (s) => (
              <Dropdown>
                ...
                <DropdownMenu aria-label="Aksi Mapel" onAction={(key) => {
                  ...
                  if (key === 'import') { setImportSubject(s); setImportOpen(true); }
                  ...
                  <DropdownItem key="import" startContent={<FiUpload />}>Import Excel</DropdownItem>
          ...
          toolbarRight={<Button color="primary" onPress={openCreateSubject}>Tambah Mapel</Button>}
        />
      )}
            ...
            {() => (
              <>
                ...
                <ModalBody className="grid grid-cols-1 md:grid-cols-2 gap-4">
                  ...
                  <Input isReadOnly label="Sekolah" labelPlacement="outside" value={schools.find((s) => s.id === schoolId)?.name || "-"} />
                </ModalBody>
                <ModalFooter>
      ...
      <ConfirmModal
        ...
        onConfirm={async () => {
          if (!target) return;
          try {
            await axios.delete(`/api/subjects/${target.id}`);
            setReloadKey((k) => k + 1);
      ...
      <QuestionImportModal isOpen={importOpen} onOpenChange={setImportOpen} subjectId={importSubject?.id ?? null} subjectName={importSubject?.name} onImported={() => setReloadKey((k) => k + 1)} />
